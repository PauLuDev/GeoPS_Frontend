...
import {PaymentModal} from "@/features/campaigns/presentation/components/PaymentModal.tsx";
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden", background: "var(--bg)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", borderBottom: "1px solid var(--line)", background: "var(--bg-elev)", flexShrink: 0 }}>
                <button type="button" className="btn btn-icon btn-sm" onClick={onBack} title="Volver">
                    <Icon name="arrowLeft" size={16}/>
                </button>
                <span style={{ fontSize: 12, color: "var(--ink-3)", flex: 1 }}>Detalle del cupón</span>
                <button type="button" className="btn btn-icon btn-sm" onClick={onToggleFav}
                        style={{ color: isFav ? "var(--brand-strong)" : "var(--ink-2)" }}>
                    <Icon name="bookmark" size={16} filled={isFav}/>
                </button>
                <button type="button" className="btn btn-icon btn-sm" onClick={handleShare}
            ...
            <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
              <Icon name="star" size={12} filled/> <strong style={{ color: "var(--ink)" }}>{c.rating}</strong> ({c.reviews} reseñas)
                    ...
                    <button type="button" className="btn btn-lg" style={{ width: "100%", justifyContent: "center", marginTop: 14, gap: 10 }} onClick={handleRoute}>
                    ...
                    <div style={{ marginTop: 24, borderTop: "1px solid var(--line)", paddingTop: 20 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                            {restrictions.map((r) => (
                                <div key={r} style={{ display: "flex", alignItems: "flex-start", gap: 10, fontSize: 13, color: "var(--ink-2)" }}>
                    ...
                    <div style={{ marginTop: 20, borderTop: "1px solid var(--line)", paddingTop: 16 }}>
                        <button type="button" style={{ width: "100%", appearance: "none", border: 0, background: "transparent", cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "4px 0", color: "var(--ink)" }}
                    ...
                    <div style={{ marginTop: 20, borderTop: "1px solid var(--line)", paddingTop: 20 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                            {reviews.map((r) => (
                                <div key={r.name} style={{ padding: 14, background: "var(--bg-elev)", border: "1px solid var(--line)", borderRadius: 10 }}>
            ...
            <div style={{ padding: "12px 16px", borderTop: "1px solid var(--line)", background: "var(--bg-elev)", flexShrink: 0 }}>
                {isReserved ? (
                    <button type="button" className="btn btn-primary btn-lg" style={{ width: "100%", justifyContent: "center" }} onClick={onBack}>
                        Listo · ir al local <Icon name="walking" size={16}/>
                    </button>
                ) : (
                    <button type="button" className="btn btn-brand btn-lg" style={{ width: "100%", justifyContent: "center" }}
